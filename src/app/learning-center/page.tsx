...
export default function LearningCenterPage() {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="min-h-full p-6 md:p-8 max-w-7xl mx-auto flex flex-col lg:flex-row gap-10"
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <motion.div variants={itemVariants} className="mb-10">
          ...
          <p className="text-text-muted text-[15px] font-light mt-1 max-w-lg">
        ...
        <motion.div variants={itemVariants} className="mb-8 w-full overflow-x-auto scrollbar-hide pb-2">
          <div className="flex gap-1.5 p-1.5 rounded-sm bg-surface border border-border/40 w-fit min-w-max shadow-sm">
            {tabs.map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={`relative flex items-center gap-2 px-5 py-2.5 rounded-sm text-[14px] font-medium transition-all duration-300 ${
                  activeTab === tab.key
                    ? "text-white"
      ...
      <div className="w-full lg:w-[320px] xl:w-[360px] shrink-0 space-y-6 lg:sticky lg:top-8 self-start">
        {/* Mastery Ring */}
        <motion.div variants={itemVariants} className="flex flex-col items-center gap-2 shrink-0 bg-surface/50 p-8 rounded-sm border border-border/40 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-500/5 blur-[40px] rounded-sm pointer-events-none translate-x-1/2 -translate-y-1/2" />
          <MasteryRing value={78} />
          <div className="flex items-center gap-1.5 text-[12px] font-medium text-text-muted uppercase tracking-widest mt-4 relative z-10">
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-2 gap-4">
          {stats.map((s) => (
            <div key={s.label} className="flex flex-col items-start gap-3 p-4 rounded-sm border border-border/40 bg-surface shadow-sm">
              <div className={`w-10 h-10 rounded-sm ${s.bg} flex items-center justify-center shrink-0 ${s.color}`}>
                {s.icon}
              </div>
              <div className="min-w-0">
                <div className="text-[20px] font-medium text-foreground leading-tight mb-0.5">{s.value}</div>
                <div className="text-[11px] text-text-muted font-medium uppercase tracking-wider">{s.label}</div>
              </div>
            </div>
          ))}
        </motion.div>
