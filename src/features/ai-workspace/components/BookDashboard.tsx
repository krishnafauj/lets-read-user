    ...
    <div className="flex-1 overflow-y-auto bg-background min-h-0 relative">
      ...
      <div className="max-w-7xl mx-auto px-8 py-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-10 items-stretch mb-16">
          ...
          <div className="w-56 h-56 rounded-xl bg-gradient-to-br from-primary/80 to-primary shadow-lg flex-shrink-0 flex items-center justify-center border border-primary/20 relative overflow-hidden group cursor-pointer hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="flex flex-col flex-1 py-1">
            <div className="flex flex-wrap items-center gap-3 mb-3">
              <span className="px-2.5 py-0.5 rounded-md bg-primary/10 text-primary text-[11px] font-medium uppercase tracking-wider">Productivity</span>
              <span className="px-2.5 py-0.5 rounded-md bg-surface-hover text-text-muted text-[11px] font-medium uppercase tracking-wider">Psychology</span>
            ...
            <h1 className="text-3xl font-semibold text-foreground/90 mb-1 tracking-tight">{bookName}</h1>
            <p className="text-[16px] text-text-muted mb-3 font-medium">Rules for Focused Success in a Distracted World</p>
            
            <p className="text-[14px] leading-relaxed text-text-muted max-w-2xl font-light">
            ...
            <div className="flex items-center gap-3 mt-auto pt-2">
              <button 
                ...
                className="flex items-center gap-2 bg-primary hover:bg-primary-hover text-white px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm"
              >
                <Plus className="w-4 h-4" />
                Start AI Conversation
              </button>
              <button className="flex items-center gap-2 bg-surface hover:bg-surface-hover border border-border text-text-muted hover:text-foreground px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-sm">
        ...
        <Tabs.Root defaultValue="explore" className="w-full">
          <Tabs.List className="flex gap-8 border-b border-border mb-6">
            ...
            <Tabs.Trigger value="activity" className="pb-3 text-[15px] font-medium text-text-muted hover:text-foreground data-[state=active]:text-primary data-[state=active]:border-b-2 data-[state=active]:border-primary transition-all outline-none">
              Activity & Stats
            </Tabs.Trigger>
          </Tabs.List>
          ...
          <Tabs.Content value="explore" className="outline-none">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div className="relative w-full max-w-sm">
                ...
                <input type="text" placeholder="Search chapters, topics, or sections..." className="w-full pl-9 pr-4 py-2 text-[13px] rounded-lg bg-surface border border-border focus:border-primary focus:outline-none transition-all shadow-sm font-light" />
              </div>
              <div className="flex items-center gap-2 text-[13px] text-text-muted font-medium">
                <button className="px-3 py-1.5 rounded-md hover:bg-surface-hover transition-colors">Expand All</button>
                <span className="text-border">|</span>
                <button className="px-3 py-1.5 rounded-md hover:bg-surface-hover transition-colors">Collapse All</button>
              </div>
            </div>
                ...
                <div key={i} className={`p-4 flex flex-col md:flex-row md:items-start justify-between gap-4 hover:bg-surface-hover/30 transition-colors group ${i < chapters.length - 1 ? 'border-b border-border' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 md:opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      ...
                      className="text-[12px] font-medium text-text-muted hover:text-primary bg-background border border-border hover:border-primary/30 px-3 py-1.5 rounded-md transition-all flex items-center gap-1.5"
                    >
                    ...
                    <button 
                      onClick={() => router.push(`/ai-workspace/${bookId}/chat?topic=${encodeURIComponent(chapter.title)}`)}
                      className="text-[12px] font-medium text-white bg-primary hover:bg-primary-hover px-3 py-1.5 rounded-md transition-all flex items-center gap-1.5 shadow-sm"
                    >
          ...
          <Tabs.Content value="actions" className="outline-none">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <button key={i} className="text-left p-6 rounded-2xl border border-border bg-surface hover:bg-surface-hover hover:border-primary/40 transition-all shadow-sm group">
              ...
              <section className="bg-surface border border-border rounded-2xl p-6 shadow-sm">
                ...
                <div className="mb-8">
                  <div className="flex justify-between items-end mb-2">
                    <span className="text-[14px] text-text-muted font-medium">Overall Progress</span>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center pb-4 border-b border-border">
                    <span className="text-[14px] text-text-muted">Pages Indexed</span>
                    <span className="text-[14px] font-bold text-foreground">847</span>
                  </div>
                  <div className="flex justify-between items-center pb-4 border-b border-border">
                    <span className="text-[14px] text-text-muted">Total Chats</span>
                    <span className="text-[14px] font-bold text-foreground">12</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[14px] text-text-muted">Highlights Extracted</span>
                    <span className="text-[14px] font-bold text-foreground">34</span>
                  </div>
                </div>
              </section>
                ...
                <div className="space-y-3">
                  {recentChats.map((chat, i) => (
                    <button 
                      ...
                      className="w-full text-left p-3 rounded-xl hover:bg-surface-hover transition-colors group flex items-start gap-3"
                    >
                      <div className="mt-0.5 p-1.5 rounded-lg bg-primary/10 text-primary">
                        <MessageSquare className="w-4 h-4" />
                      </div>
                      <div>
                        <p className="text-[14px] font-medium text-foreground group-hover:text-primary transition-colors line-clamp-1 mb-0.5">{chat.title}</p>
                        <p className="text-[12px] text-text-muted">{chat.time}</p>
                      </div>
                    </button>
                  ))}
                </div>
                <button className="w-full mt-4 py-2 text-center text-[13px] font-semibold text-primary hover:bg-primary/5 rounded-lg transition-colors">
                  View All History
                </button>
              </section>
            </div>
