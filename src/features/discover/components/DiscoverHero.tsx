...
export const DiscoverHero = () => {
  ...
  return (
    <motion.section 
      ...
      className={`relative z-50 w-full rounded-3xl bg-surface border border-border px-8 md:px-16 mb-12 flex flex-col items-center justify-center text-center shadow-2xl ${query ? 'min-h-[160px] py-10' : 'min-h-[400px] py-16'}`}
    >
      ...
      <div className="relative z-10 max-w-2xl w-full flex flex-col items-center w-full">
        <AnimatePresence>
          {!query && (
            ...
            >
              ...
              <h1 className="text-4xl md:text-6xl font-semibold text-foreground mb-6 tracking-tight leading-tight">
        ...
        <motion.div layout className="relative w-full max-w-xl group z-50 text-left">
          <form onSubmit={handleSearch} className="relative w-full">
            <div className={`absolute -inset-1 bg-gradient-to-r from-primary via-secondary to-warning blur opacity-20 transition duration-1000 ${isFocused ? 'opacity-40 rounded-3xl' : 'group-hover:opacity-40 rounded-3xl'}`} />
            <div className={`relative flex items-center bg-background/90 backdrop-blur-2xl border border-white/10 ${isFocused && query ? 'rounded-t-xl rounded-b-none border-b-0' : 'rounded-xl'} px-6 py-4 shadow-xl transition-all duration-300`}>
              ...
              />
              <button type="submit" className="bg-primary text-white px-6 py-2 rounded-xl font-semibold text-sm hover:bg-primary-dark transition-colors shadow-lg shadow-primary/20 ml-2 shrink-0">
                Search
              </button>
            </div>
          </form>
          ...
          <AnimatePresence>
            {isFocused && query && (
              <motion.div
                ...
                className="absolute top-full left-0 w-full bg-background/90 backdrop-blur-2xl border border-white/10 border-t-0 rounded-b-xl shadow-2xl overflow-hidden flex flex-col"
              >
                <div className="p-4 grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-hover cursor-pointer transition-colors" onClick={() => router.push('/search?q=1984')}>
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">1984</p>
                          <p className="text-xs text-text-muted">George Orwell</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-hover cursor-pointer transition-colors" onClick={() => router.push('/search?q=Dune')}>
                        <div className="w-8 h-12 bg-surface rounded shadow-sm border border-border overflow-hidden shrink-0">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">Dune</p>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-hover cursor-pointer transition-colors" onClick={() => router.push('/search?q=George R.R. Martin&tab=authors')}>
                        <div className="w-10 h-10 bg-surface rounded-lg shadow-sm border border-border overflow-hidden shrink-0">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">George R.R. Martin</p>
                          <p className="text-xs text-text-muted">Epic Fantasy</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-hover cursor-pointer transition-colors" onClick={() => router.push('/search?q=J.K. Rowling&tab=authors')}>
                        <div className="w-10 h-10 bg-surface rounded-lg shadow-sm border border-border overflow-hidden shrink-0">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">J.K. Rowling</p>
